...
namespace split
{
    class Split
    {
		...
		static int next_split(int n, int need_gui, int max_gui)
		{
			int c=0;
			while(true){
		...
		static int one(int n, int need_gui, int max_gui)
		{
			int c1 = n % 10;
			int c2 = (n % 100) / 10;
		...
		static int two(int n, int need_gui, int max_gui)
		{
			int c1 = n % 10;
			int c2 = (n % 100) / 10;
			int c3 = (n % 1000) / 100;
	    ...
	    static int check_zi(char* cards,  int max_gui, int cache_index, int* cache)
		{
			if (cache_index >= 0) {
				int n = cache[cache_index];
				if(n>0) return n - 1;
