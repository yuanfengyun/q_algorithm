...
{		
	...
	stAnswer stResult;
	bool bSuc = CanWin2(stData, stResult, byNaiZi, mapHuanCun);
//	if (bSuc)
	if (false)
	{
		int nJiangNum = 0;
		vctNodeOut = stResult.getVctNode();
...
bool CHuTipsMJ::CheckWin_7XiaoDui(stCardData stData, std::vector<stNodeMJ>& vctNodeOut, BYTE byOutCard, BYTE byNaiziCard)
{
	...
	BYTE byIndexNai = getIndexByVal(byNaiziCard);
	BYTE nNaiZi = 0;
	if (byIndexNai < MAX_TOTAL_TYPE)	
		nNaiZi = stData.byCardCount[byIndexNai];
	...
	for (int i=0; i<MAX_TOTAL_TYPE; ++i)
	{
		if (i != byIndexNai)
		{
			while (stData.byCardCount[i] >= 2)
			{
				stNodeMJ stNode;
				stNode.byType = BLOCK_22;
				stNode.addCardInfo(getValByIndex(i), 2);
				vctNodeOut.push_back(stNode);
				stData.DelCard(stNode);
			}
		}
	}
	//2.将单个的与赖子配对
	if (nNaiZi > 0)
	{
		for (int i=0; i<MAX_TOTAL_TYPE; ++i)
		{
			if (i != byIndexNai)
			{
				if (stData.byCardCount[i] == 1)
				{
					stNodeMJ stNode;
					stNode.byType = BLOCK_22;
					stNode.addCardInfo(getValByIndex(i), 1);
					stNode.addCardInfo(byNaiziCard, 1);
					vctNodeOut.push_back(stNode);
					stData.DelCard(stNode);
					--nNaiZi;
				}
	...
	while (nNaiZi >= 2)
	{
		stNodeMJ stNode;
		stNode.byType = BLOCK_22;
		stNode.addCardInfo(byNaiziCard, 2);
		vctNodeOut.push_back(stNode);
		stData.DelCard(stNode);
		nNaiZi -= 2;
	}

	return vctNodeOut.size() == 7;
}
...
bool CHuTipsMJ::CanWin2(stCardData &stData, stAnswer& stResult, BYTE byIndexNaiZi, map<stKey, stAnswer> &mapHuanCun)
{
	BYTE nNaiZi = 0;
	...
	if (nNaiZi == 0 && stResult.nNum == 0)
		...
		return CanWin_Do_Nai2(stData, stResult, byIndexNaiZi, mapHuanCun);
	return false;
}
