...
public:	
	...
	bool CanWin2(stCardData &stData, stAnswer& stResult, BYTE byIndexNaiZi, map<stKey, stAnswer> &mapHuanCun);

	// 无赖子时，分颜色计算胡牌
	bool CanWin_Do(stCardData &stData, stAnswer& stResult);
	bool ColorMatch(stColorData &stData, stAnswer& stResult, int nFlag=0);
	// 有赖子时，分颜色计算胡牌
	bool CanWin_Do_Nai(stCardData &stData, stAnswer& stResult, BYTE byIndexNaiZi = INVALID_VAL);
	bool CanWin_Do_Nai2(stCardData &stData, stAnswer& stResult, BYTE byIndexNaiZi, map<stKey, stAnswer> &mapHuanCun);
	bool ColorMatch2(stColorData &stData, stAnswer& stResult, int nFlag=0, BYTE byCardNaiZi = INVALID_VAL, BYTE byNumNaiZi = 0);
