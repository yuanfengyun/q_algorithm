...
struct stAnswer
{
	...
	{
		if (answerTemp.nNum == 0 || nNum + answerTemp.nNum > 4)
			return false;
	...
	vector<stNodeMJ> getVctNode()
	{
		vector<stNodeMJ> vctTemp;
		for (BYTE i=0; i<nNum; ++i)
			vctTemp.push_back(stNode[i]);
